...
import jp.techacademy.tomotaka.aruga.apiapp.databinding.FragmentApiBinding
import okhttp3.*
import okhttp3.logging.HttpLoggingInterceptor
import java.io.IOException

//このフラグメントは、APIから取得したデータを一覧表示し、各アイテムに対してお気に入りの追加・削除や詳細表示などの操作を提供します。
// また、スクロールによる追加読み込み（ページネーション）やスワイプによるリフレッシュもサポートしています。
...
//    フラグメントがアクティビティにアタッチされたときに呼ばれるメソッドです。
//    ここでは、アクティビティがFragmentCallbackインターフェースを実装しているかをチェックし、
//    実装していればそのインスタンスをfragmentCallbackに保持しています。
...
//    フラグメントのビューが作成されるときに呼ばれるメソッドです。
//    ここでは、ViewBindingを使用してレイアウトをインフレートし、
//    そのルートビューを返しています
...
//    フラグメントのビューが作成された後に呼ばれるメソッドです。
//    ここでは、アダプターの初期化やRecyclerViewの設定、
//    スワイプリフレッシュのリスナー設定などを行っています。
...
//    RecyclerViewのアダプターにデータをセットし、
//    スワイプリフレッシュのインジケータを非表示にするメソッドです。
        // RecyclerViewの初期化
    ...
    //APIからデータを取得し、それを表示するためのメソッドです。
//    ここでは、APIからデータを非同期に取得し、
//    その結果をRecyclerViewのアダプターにセットしています。
    /**
