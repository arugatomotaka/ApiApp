...
//このファイルは、Androidアプリケーションでリスト表示を行うためのAdapterとViewHolderを定義しています。
// 具体的には、ShopというデータクラスのリストをRecyclerViewで表示するための,
// Adapter（ApiAdapter）とViewHolder（ApiItemViewHolder）を定義しています。
...
class ApiItemViewHolder(private val binding: RecyclerFavoriteBinding) :
    ...
    fun bind(shop: Shop, position: Int, adapter: ApiAdapter) {
        binding.rootView.apply {
            ...
            //position÷2の余りが0の時すなわち偶数の時にtrueを返す
            )
            setOnClickListener {
